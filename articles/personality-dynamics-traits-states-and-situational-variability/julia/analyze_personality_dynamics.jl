...
raw = readdlm(joinpath(root, "data", "synthetic_personality_dynamics_data.csv"), ',', String)
header = vec(raw[1, :]); rows = raw[2:end, :]
col(name) = findfirst(==(name), header)
contexts = unique(rows[:, col("assessment_context")])
metrics = ["state_extraversion", "state_conscientiousness", "state_neuroticism", "situation_sociality", "situation_demand", "dynamic_signature_score"]
